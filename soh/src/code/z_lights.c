...
void Lights_PointSetColorAndRadius(LightInfo* info, u8 r, u8 g, u8 b, s16 radius) {
    // SOH [Enhancement] Apply the Wind Waker flame flicker at the source whenever light casting is enabled,
    // so it reaches the scene lighting, the vanilla glow, and the cast light pools alike. The helper only
    // transforms lights that are actively flickering (it detects steady/smooth lights and leaves them alone).
    if (CVarGetInteger("gEnhancements.Graphics.WorldLighting.Enabled", 0)) {
...
void Lights_DrawGlow(PlayState* play) {
    ...
    // otherwise clash with the cast light pools). Only while light casting is enabled, and unless the
    // user re-enables the vanilla glow. (Selection/draw live in soh/soh/Enhancements/Graphics/WorldLighting.cpp.)
    if (CVarGetInteger("gEnhancements.Graphics.WorldLighting.Enabled", 0) &&
        !CVarGetInteger("gEnhancements.Graphics.WorldLighting.ShowVanillaGlow", 0)) {
        return;
    }

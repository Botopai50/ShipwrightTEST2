...
1. The release includes a starter mod file called **`ww_clouds.o2r`**. Open it in
   **[Retro](https://github.com/HarbourMasters/retro)** (the community tool for unpacking and
   packing o2r files) — it unpacks into a folder of textures.
2. Replace the five cloud textures with your own. **Keep the same names and folder locations.**
3. Repack it with Retro and drop the new `.o2r` into your game's **`mods`** folder.
...
*Developer notes: the built-in textures are generated by `scripts/gen-ww-cloud-textures.py` into
[`soh/assets/custom/textures/wind-waker/clouds/`](../soh/assets/custom/textures/wind-waker/clouds/)
and packed into `soh.o2r` at build time; a mods-folder o2r with the same resource paths overrides
them. The release's starter `ww_clouds.o2r` is those same PNGs packed standalone (ZAPD custom-o2r —
command in
...
Never bundle an o2r containing textures extracted from a real Wind Waker ROM.*
